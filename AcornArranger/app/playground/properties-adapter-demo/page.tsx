...
export default function PropertiesAdapterDemo() {
  const [q, setQ] = React.useState("");
  const [page, setPage] = React.useState(1);
  const [pageSize, setPageSize] = React.useState(10);
  const [sort, setSort] = React.useState<Array<{ id: string; desc: boolean }>>([]);

  const params = React.useMemo(() => PropertiesAdapter.toApiParams({
  ...
  const url = React.useMemo(() => {
    ...
    return qs ? `${PropertiesAdapter.endpoint}?${qs}` : PropertiesAdapter.endpoint;
  }, [params]);

  return (
    <div className="p-6 space-y-4">
      <h1 className="text-2xl font-semibold">Properties Adapter Demo</h1>
      ...
      <Separator />
      <TablePagination page={page} pageSize={pageSize} total={520} onPageChange={setPage} onPageSizeChange={setPageSize} />
      <p className="text-sm text-muted-foreground">This demo shows the computed API URL using the adapter. All query params use camelCase (pageSize, statusIds, etc).</p>
    </div>
  );
}
