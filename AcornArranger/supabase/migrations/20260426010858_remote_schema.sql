...
CREATE OR REPLACE FUNCTION public.commit_schedule_plan(p_plan_date date, p_plan jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SET search_path TO ''
AS $function$
...
CREATE OR REPLACE FUNCTION public.get_staff_pairing_affinity(p_lookback_days integer DEFAULT 90)
 RETURNS TABLE(staff_a_id bigint, staff_b_id bigint, score numeric)
 LANGUAGE sql
 STABLE
 SET search_path TO ''
AS $function$
...
CREATE OR REPLACE FUNCTION public.get_staff_property_affinity(p_lookback_days integer DEFAULT 180)
 RETURNS TABLE(staff_id bigint, property_id bigint, score numeric)
 LANGUAGE sql
 STABLE
 SET search_path TO ''
AS $function$
