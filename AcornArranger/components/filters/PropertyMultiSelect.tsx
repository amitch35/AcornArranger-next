...
type PropertyMultiSelectProps = {
  ...
  // Optional remote loading support: when provided, the component
  // will invoke loadOptions with backend filters.
  loadOptions?: (filters: { q?: string; city?: string; statusIds?: number[] }) => void;
  // External filters (controlled by parent)
  city?: string;
  statusIds?: number[];
  onlyActive?: boolean; // convenience: if true, maps to statusIds=[1]
};
...
export function PropertyMultiSelect({
  ...
  React.useEffect(() => {
    ...
    const controller = setTimeout(() => {
      const effectiveStatusIds = onlyActive ? [1] : statusIds;
  ...
  return (
    <div className={className}>
      <div className="flex items-center gap-2">
        ...
        <Button variant="secondary" disabled={value.length === 0 || disabled} onClick={clearAll} aria-label="Clear selected properties">
          Clear
        </Button>
      </div>
          ...
          {value.map((val) => {
            ...
            return (
              <Badge key={val} variant="secondary" className="gap-1">
                {labelText}
                <button
                  aria-label={`Remove property ${labelText}`}
